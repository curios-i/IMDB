...
We are not covering the installation of Cuda, cuDNN, tensorflow and keras in this report, though it would have been an interesting topic considering very little and correct information is available on this topic for RStudio; most of the guidelines are for python/anaconda applications.
...
The2D convolutions extract 2D patches from image tensors and applying an identical transformation to every patch. In
the same way, you can use 1D convolutions, extracting local 1D patches (sub-sequences) from sequences.
...
of RNNs is to use a 1D convnet as a preprocessing step before an RNN (See Fig)
